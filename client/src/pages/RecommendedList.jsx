...
const RecommendedList = () => {
  ...
  const FetchRecommendedList = async () => {
    const result = await axios.get(`/recommendall?page=${page}`);
    return result.data.data;
  };

  const { data: list, isLoading } = useQuery(["recom", page], ()=> FetchRecommendedList(page));
  ...
  return (
    <>
    <Helmet>
        <title>Manga Swipe - Recommended Manga</title>
    ...
    <div className="text-center">
      <h1 className="text-5xl my-[25px] uppercase">
        Recommended Manga{" "}
      </h1>
