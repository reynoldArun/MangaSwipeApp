...
const RecentlyUpdateList = () => {
  const [page, setPage] = useState(0)

  const FetchRecentlyUpdateList = async () => {
    const result = await axios.get(`/recentlist?page=${page}`);
    return result.data.data;
  };
  const { data: list, isLoading } = useQuery(["recentlist", page], ()=>FetchRecentlyUpdateList(page));
  ...
  return (
    <>
     <Helmet>
        <title>Manga Swipe - Recently Update</title>
    ...
    <div className="text-center">
      <h1 className="text-5xl my-[25px]">Recently Update Reads</h1>
...
export default RecentlyUpdateList;
